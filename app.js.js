...
bot.on('ready', () => {
  console.log('I am ready!/yes /yes /yes maher bot online');
});
bot.login('Mjg0NDMyNjgzMjk0MTMwMTc3.C6RBng.JXeraDaU4RJ-LvSgmsGoqXk_EBY');

  bot.on('message', message => {
    if (message.content.startsWith('!' + 'level')) {
    message.reply(`You are currently level , with  points.`);
  }
  });
...
bot.on('guildCreate', guild => {
  guild.defaultChannel.sendMessage(`I have joined ${guild.name} if you Have problem https://discord.gg/EXkkUaj and visit my website https://mahervahmed.wixsite.com/maher-bot`);
});
...
bot.on('message', message => {
  if (message.content.startsWith('!' + 'boo')) {
	      message.channel.sendMessage(`Updated the content of a message from ${message.author}`);
  } 
  });

  bot.on('message', message => {
  if (message.content.startsWith('+play')) {
    const TextChannels = message.member.voiceChannel;
    if (!TextChannels) {
...
bot.on('message', (message) => {
  if(message.content === '!say') {
    message.channel.sendMessage('what would i say?');
  }
});
...
//////////
//bot.on("message", msg => {
  //msg.channel.send(msg.member +':rofl:  .HELLO!	')
    //msg.member.kick();
//})
///////
bot.on('message', message => {
  if (message.content.startsWith('!' + 'kick')) {
    if (!message.mentions.users) return message.reply("please mention someone to kick");
    let memberToKick = message.guild.member(message.mentions.users.first());
    message.channel.sendMessage(memberToKick + "is kicked!");
    memberToKick.kick();
  } 
  });
  ...
  if (message.content.startsWith('!' + 'ban')) {
    if (!message.mentions.users) return message.reply("please mention someone to ban");
    let memberToban = message.guild.member(message.mentions.users.first());
    message.channel.sendMessage(memberToban + "is baned!");
    memberToban.ban();
  } 
  });
bot.on('messageDelete', msg => {
    msg.guild.defaultChannel.sendMessage("", { embed: {
        color: 3447003,
        author: {
        ...
        description: 'History of deleted message',
        fields: [
            {
                name: 'Where',
                value: `${msg.channel}`
...
bot.on("message", (message) => {
    ...
    if (message.content.startsWith('!'+ "maher")) {
...
bot.on('message', message => {
  if (message.content.startsWith('!' + 'game')) {
    if (!message.content.split(" ")[1]) return message.reply("Give me something to rate.");
    const RatingList = ["1/10 :carrot: ", "2/10 :carrot: ", "3/10:carrot: ", "4/10 :carrot: ", "5/10 :carrot: ", "6/10 :carrot: ", "7/10 :carrot: ", "8/10 :carrot: ", "9/10:carrot:", "10/10 :carrot: "];
    const Rating = RatingList[Math.floor(Math.random() * RatingList.length)];
    message.channel.sendMessage("I give " + message.content.split(" ")[1] + "a " + Rating);
} 
  });
bot.on('guildCreate', guild => guild.createChannel('modlog', 'text'));
bot.on('guildCreate', guild => guild.createChannel('welcome', 'text'))
bot.on('guildMemberAdd', member => {
  const welcomelog = ('name', 'welcome');
  if (!welcomelog) {
    return;
  
  const embed = new Discord.RichEmbed()
  .setTitle(`User joined`)
  .setAuthor(`${client.user.username}`, `https://images-ext-2.discordapp.net/eyJ1cmwiOiJodHRwczovL2dvby5nbC9sTE1nMnIifQ.urA2aHQ5E-60ZPOQqyp6ILUSgyo`)
  .setColor(0x904098)
  .setFooter('Knight Utility')
  .setThumbnail(`${client.user.avatarURL}`)
  .setTimestamp()
  .addField(`Welcome ${member.username} to ${member.member.username}`, `${member.username} was invited by ${member.inviter}`);
  welcomelog.sendEmbed(embed);
}
});

bot.on('message', message => {
  if (message.content.startsWith('!' + 'Deleted')) {
    let messagecount = parseInt([0]);
    message.channel.fetchMessages({limit: messagecount}).then(msg =>
...
//bot.on('message', message => {
  //  var args = message.content.split(/[ ]+/)
    //if(message.content.includes('discord.gg')){
      //  message.delete()
    //return message.reply(' ** ممنوع ارسال اي انفايت بهذا السيرفر ** ' +  ' ' + '' + '!')
  
//}
//});
  if (message === "مسح") {
  let modRole = message.guild.roles.find("name", "MFLS3YN");
  if (!modRole) return message.reply('يجب ان يكون لديك رتبة : MFLS3YN').catch(console.error); {
    }
...
bot.on('message', message => {
  if (message.content.startsWith('!' + 'help'))
		message.author.send('"**hello Thats help do you want**``` \nto ban use----> !ban mention\nto kick someone use---->!kick mention \nto see your ping use ---->!ping\nto play Video----> +play\nto Deleted the message use----> !Deleted```"')
		
  });
  const butts = new Discord.Client()
  
  bot.on('message', message => {
  if (message.content === '//') {
  message.channel.send('**hello @everyone you shoud read this rule first!**');
  }
});
  bot.on('message', message => {
  if (message.content === 'rule 1') {
  message.channel.send('**1- Not insulting**\nYou can not insult and insult anyone');
  }
});
  bot.on('message', message => {
  if (message.content === 'rule 2') {
  message.channel.send('**2 - prevent controversy and quarrel with another member with you**\nIn the team and you must inform the team leader or the deputy of any problem caused by another member');
  }
});
.
